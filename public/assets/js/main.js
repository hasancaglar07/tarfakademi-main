...
(function ($) {
    ...
    const ThreeDLetterMenuEffect = () => {
        $(".threeD-letter-menu .menu-item").each(function () {
            ...
            // Logo öğelerini etkilememek için kontrol ekle
            if (MenuLink && MenuText && !MenuLink.closest('.navbar-brand')) {
            ...
            if (hasPos > -1) {
                ...
                if (res != '' && res != '#' && res.length > 1) {
                    try {
                        ...
                        // Geçersiz jQuery seçiciyi sessizce yok say
                        console.warn('Invalid jQuery selector:', res);
                    }
                }
            }
        });
    ...
    function setupSwiper() {
        ...
        // Mobile Services Tabs Swiper
        var mobileServicesContainer = document.querySelector('.mobile-swipe-container');
        if (mobileServicesContainer && typeof Swiper === 'function') {
            var mobileServicesSwiper = new Swiper('.mobile-swipe-wrapper', {
                autoplay: {
                    delay: 4000,
                ...
                // Navigation
                navigation: {
                    nextEl: '.mobile-swipe-next',
                    prevEl: '.mobile-swipe-prev',
                },
                ...
                on: {
                    init: function () {
                        updateMobileServicesActiveStates();
                        hideSwipeHints();
                    },
                    slideChange: function () {
                        updateMobileServicesActiveStates();
            ...
            function updateMobileServicesActiveStates() {
                if (!mobileServicesSwiper) return;

                var slides = document.querySelectorAll('.mobile-swipe-slide');
                var activeIndex = mobileServicesSwiper.activeIndex;
                ...
                // Update progress bar
                var progressBar = document.querySelector('.mobile-swipe-progress-bar');
                if (progressBar) {
            ...
            function hideSwipeHints() {
                var hints = document.querySelectorAll('.swipe-hint-overlay');
                hints.forEach(function(hint) {
                    hint.style.opacity = '0';
                });
            }

            // Disable Swiper on desktop and enable on mobile
            function handleMobileServicesResize() {
                if (!mobileServicesSwiper) return;

                if (getWindowWidth() >= 768) {
                    mobileServicesSwiper.disable();
                } else {
                    mobileServicesSwiper.enable();
                }
            }

            // Listen for resize events
            window.addEventListener('resize', handleMobileServicesResize);

            // Initial check
            handleMobileServicesResize();
...
$(document).ready(function() {
    // Blog tabs functionality
    $('#blogTabs button[data-bs-toggle="pill"]').on('shown.bs.tab', function(event) {
        // Tab değiştiğinde grid animasyonunu yeniden başlat
        var targetPane = $(event.target.getAttribute('data-bs-target'));
        if (targetPane.length) {
            var gridItems = targetPane.find('.grid-item');
...
document.addEventListener('DOMContentLoaded', function () {
    // Sadece mobil cihazlarda çalışacak Swiper'ı seç
    const mobileSwiperEl = document.querySelector('.mobile-swipe-wrapper');
    if (mobileSwiperEl) {
        ...
        const mobileSwiper = new Swiper(mobileSwiperEl, {
            // Bir sonraki slaytın görünmesini sağlayan ayarlar
            slidesPerView: 1.15,
            spaceBetween: 15,
            centeredSlides: true,
            ...
            on: {
                // Autoplay ilerlemesini göstermek için
                autoplayTimeLeft(s, time, progress) {
                    if (progressCircle) {
                        progressCircle.style.setProperty("--progress", 1 - progress);
                    }
                    if (progressContent) {
                        // Geri sayım saniyesini göstermek isterseniz
                        // progressContent.textContent = `${Math.ceil(time / 1000)}s`;
                    }
                },

                // Kullanıcı ilk kaydırmayı yaptığında ipucunu gizle
                slideChange: function () {
                    const hintOverlay = document.querySelector('.swipe-hint-overlay-modern');
                    if (hintOverlay && !hintOverlay.classList.contains('is-hidden')) {
                        hintOverlay.classList.add('is-hidden');
                    }
                },

                // Swiper başladığında ipucunu birkaç saniye sonra gizle
                init: function () {
                    setTimeout(() => {
                        const hintOverlay = document.querySelector('.swipe-hint-overlay-modern');
                        if (hintOverlay && !hintOverlay.classList.contains('is-hidden')) {
